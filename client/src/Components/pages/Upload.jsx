...
const Upload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      let photoLink = `https://${cid}.ipfs.dweb.link/${encodeURIComponent(
        photo.name
      )}`;
      let audioLink = `https://${cid}.ipfs.dweb.link/${encodeURIComponent(
        audio.name
      )}`;
      return { photoLink, audioLink };
    } catch (error) {
